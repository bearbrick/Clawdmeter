...
**Weekly reset weekday (`wd` / `xwd`).** A "6d 23h" countdown never says *which day* the weekly window rolls over, so both daemons ship the weekday alongside it (Claude's `wd` from the unified-7d reset header, Codex's `xwd` from the weekly window's `resets_at`). Two rules: the abbreviation comes from a **fixed English table, never `strftime("%a")`** — `%a` follows the host locale and a zh_CN Mac would emit "周四", glyphs the subsetted Styrene fonts don't carry, so the label renders blank; and the key is **omitted, never sent empty**, when the stamp is unparseable or already in the past (a rolled-over window's weekday names a reset that has been and gone). The firmware falls back to the bare countdown when it's absent, which is also what happens against an older daemon.

The split (Claude + Codex) reset line reads `4h 28m - Week 18% Thu` — no "Resets" prefix, dropped to buy back width. On a plan reporting only a weekly window (some Codex accounts today) that one window fills **both** halves of its panel, so the two panels always render the same shape and the layout doesn't reflow if Codex's 5-hour window returns. Widths are tight: at styrene_28 the line already overflows the 1.8 (368px) and 2.06 (410px) panels and is only safe on the 2.16 and the 1.54 — a narrower board needs the reset font dropped a step in split mode.
