...
- **Slots carry no meaning; `window_minutes` does.** Codex reports a variable set of windows and the `primary`/`secondary` slots do not map to fixed lengths. A ChatGPT Plus account on Codex 0.147 puts its **weekly** window in `primary` and leaves `secondary` null — no 5-hour window exists at all. Assuming `primary` == 5h made that whole shape read as "no data". `_map_windows()` identifies windows by length and treats one window as a valid answer, promoting it to the headline (`xs`) with `xwin` naming its length.
- **Absence is the signal.** No Codex keys in the payload ⇒ firmware keeps the original Current/Weekly view (`codex_valid` in `data.h`). Never send zeroed keys to mean "no Codex" — a real 0% is meaningful and must still split the screen. Same rule for `xw`: absent means "this plan has one window", not "the weekly window is at 0".
- **Providers degrade independently.** `add_codex_fields()` swallows every exception, so Codex can never break a Claude reading. Conversely, `codex_only_payload()` promotes the local Codex windows into the base `s`/`w` slots when Claude has no token or usable response; a missing/expired Claude login must never turn valid Codex data into `{"ok":false}`. The compact `src=codex`, `win`, and `hw` fields let new firmware label that standalone view accurately, while older firmware still shows the numbers as Current/Weekly.

**The split reset line is width-bound — measure before editing it.** It reads `4h 28m - Week 18% 6d 23h`: the headline window's countdown, then the weekly window's percentage and its own countdown. No "Resets" prefix (dropped to buy width; the panel's pill already names the window). Two non-obvious rules, both forced by measurement against the glyph tables in `font_styrene_28.c`:

- **The weekly half uses a coarser countdown than the headline** (`format_countdown_coarse`): hours-only between an hour and a day. `22h 23m` carries two 2-digit numbers and measures **427px inside a 408px panel**, where `22h` measures 358px. Minutes come back below an hour, where the string is short enough to afford them. The headline keeps full `Xh Ym` precision.
- **A provider reporting only a weekly window** (some Codex accounts today) fills the "Week" half from its single window, but passes `week_mins < 0` so the trailing countdown is dropped — otherwise the headline's countdown prints twice verbatim on one line and reads as a rendering fault.

Widths are genuinely tight: the worst case is 401px against the 2.16's 408px. At styrene_28 this line already overflows the 1.8 (296px) and 2.06 (338px) panels, as the format before it did — those boards need the reset font dropped a step in split mode. There is no room for a third field; a weekday abbreviation alongside the countdown measures 471px and was tried and removed.
